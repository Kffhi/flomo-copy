...
const hasTarget = (editor: VueEditorType, target: EventTarget | null): target is DOMNode => {
 ...
 */
const hasEditableTarget = (editor: VueEditorType, target: EventTarget | null): target is DOMNode => {
    return isDOMNode(target) && VueEditor.hasDOMNode(editor, target, { editable: true })
}
...
const isTargetInsideVoid = (editor: VueEditorType, target: EventTarget | null): boolean => {
    const slateNode = hasTarget(editor, target) && VueEditor.toSlateNode(editor, target)
...
export const EditableComponent = {
    ...
    onClick(event: any, editor: any, ctx: any) {
        if (!ctx.readOnly && hasTarget(editor, event.target) && isDOMNode(event.target) && !isEventHandled(event, ctx.onClick)) {
    ...
    onBeforeInput(event: any, editor: any, ctx: any) {
        ...
        if (!ctx.readOnly && hasEditableTarget(editor, event.target) && !isEventHandled(event, ctx.onBeforeInput)) {
    ...
    onCompositionEnd(event: any, editor: any, ctx: any) {
        if (hasEditableTarget(editor, event.target) && !isEventHandled(event, ctx.onCompositionEnd)) {
    ...
    onCompositionStart(event: any, editor: any, ctx: any) {
        if (hasEditableTarget(editor, event.target) && !isEventHandled(event, ctx.onCompositionStart)) {
            ctx.isComposing = true
        }
    },
    onKeyDown(event: any, editor: any, ctx: any) {
        if (!ctx.readOnly && hasEditableTarget(editor, event.target) && !isEventHandled(event, ctx.onKeyDown)) {
    ...
    onFocus(event: any, editor: any, ctx: any) {
        if (!ctx.readOnly && !ctx.isUpdatingSelection && hasEditableTarget(editor, event.target) && !isEventHandled(event, ctx.onFocus)) {
    ...
    onBlur(event: any, editor: any, ctx: any) {
        if (ctx.readOnly || ctx.isUpdatingSelection || !hasEditableTarget(editor, event.target) || !isEventHandled(event, ctx.onBlur)) {
            return
        }
        ...
        if (relatedTarget != null && isDOMNode(relatedTarget) && VueEditor.hasDOMNode(editor, relatedTarget)) {
    ...
    onCopy(event: any, editor: any, ctx: any) {
        if (hasEditableTarget(editor, event.target) && !isEventHandled(event, ctx.onCopy)) {
            event.preventDefault()
            VueEditor.setFragmentData(editor, event.clipboardData)
        }
    },
    onPaste(event: any, editor: any, ctx: any) {
        if ((!HAS_BEFORE_INPUT_SUPPORT || isPlainTextOnlyPaste(event)) && !ctx.readOnly && hasEditableTarget(editor, event.target) && !isEventHandled(event, ctx.onPaste)) {
            event.preventDefault()
            VueEditor.insertData(editor, event.clipboardData)
        }
    },
    onCut(event: any, editor: any, ctx: any) {
        if (!ctx.readOnly && hasEditableTarget(editor, event.target) && !isEventHandled(event, ctx.onCut)) {
            event.preventDefault()
            VueEditor.setFragmentData(editor, event.clipboardData)
            const { selection } = editor
    ...
    onDragOver(event: any, editor: any, ctx: any) {
        if (hasTarget(editor, event.target) && !isEventHandled(event, ctx.onDragOver)) {
    ...
    onDragStart(event: any, editor: any, ctx: any) {
        if (hasTarget(editor, event.target) && !isEventHandled(event, ctx.onDragStart)) {
            const node = VueEditor.toSlateNode(editor, event.target)
            const path = VueEditor.findPath(editor, node)
    ...
    onDrop(event: any, editor: any, ctx: any) {
        if (hasTarget(editor, event.target) && !ctx.readOnly && !isEventHandled(event, ctx.onDrop)) {
