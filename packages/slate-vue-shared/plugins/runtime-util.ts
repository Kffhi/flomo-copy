...
export const transform = (editor: Editor, op: Operation, Vue?: any) => {
    switch (op.type) {
        case 'insert_node': {
            const { path, node } = op
        ...
        case 'merge_node': {
            ...
            if (Text.isText(node) && Text.isText(prev)) {
                ...
                throw new Error(`Cannot apply a "merge_node" operation at path [${path}] to nodes of different interaces: ${node} ${prev}`)
            }
...
export const runtimeNode = {
    child(root: Node, index: number): Descendant {
        if (Text.isText(root)) {
            throw new Error(`Cannot get the child of a text node: ${JSON.stringify(root)}`)
        }

        const c = getChildren(root)[index] as Descendant

        if (c == null) {
            throw new Error(`Cannot get child at index \`${index}\` in node: ${JSON.stringify(root)}`)
        }
    ...
    get(root: Node, path: Path): Node {
        ...
        for (let i = 0; i < path.length; i += 1) {
            ...
            if (Text.isText(node) || !children[p]) {
                throw new Error(`Cannot find a descendant at path [${path}] in node: ${JSON.stringify(root)}`)
            }
    ...
    *nodes(
        ...
        while (true) {
            ...
            if (!visited.has(n) && !Text.isText(n) && getChildren(n).length !== 0 && (pass == null || pass([n, p]) === false)) {
                visited.add(n)
