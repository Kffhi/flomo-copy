...
export const VueEditor = {
    ...
    findKey(editor: VueEditorType, node: Node): Key {
    ...
    findPath(editor: VueEditorType, node: Node): Path {
        const path: Path = []
        let child = node
    ...
    isFocused(editor: VueEditorType): boolean {
    ...
    isReadOnly(editor: VueEditorType): boolean {
    ...
    blur(editor: VueEditorType): void {
    ...
    focus(editor: VueEditorType): void {
        const el = VueEditor.toDOMNode(editor, editor)
    ...
    deselect(editor: VueEditorType): void {
    ...
    hasDOMNode(editor: VueEditorType, target: DOMNode, options: { editable?: boolean } = {}): boolean {
    ...
    insertData(editor: VueEditorType, data: DataTransfer): void {
    ...
    setFragmentData(editor: VueEditorType, data: DataTransfer): void {
    ...
    toDOMNode(editor: VueEditorType, node: Node): HTMLElement {
        const domNode = Editor.isEditor(node) ? EDITOR_TO_ELEMENT.get(editor) : KEY_TO_ELEMENT.get(VueEditor.findKey(editor, node))
        if (!domNode) {
    ...
    toDOMPoint(editor: VueEditorType, point: Point): DOMPoint {
     ...
     *
     * Notice: the returned range will always be ordinal regardless of the direction of Slate `range` due to DOM API
     * limit.
     *
    ...
    toDOMRange(editor: VueEditorType, range: Range): DOMRange {
        const { anchor, focus } = range
        const isBackward = Range.isBackward(range)
        const domAnchor = VueEditor.toDOMPoint(editor, anchor)
        const domFocus = Range.isCollapsed(range) ? domAnchor : VueEditor.toDOMPoint(editor, focus)
    ...
    toSlateNode(editor: VueEditorType, domNode: DOMNode): Node {
    ...
    findEventRange(editor: VueEditorType, event: any): Range {
    ...
    toSlatePoint(editor: VueEditorType, domPoint: DOMPoint): Point {
        ...
        if (parentNode) {
            ...
            if (leafNode) {
                ...
                const ele1 = Array.from(contents.querySelectorAll('[data-slate-zero-width]'))
                const ele2 = Array.from(contents.querySelectorAll('[contenteditable=false]'))
                const removals: Element[] = [...ele1, ...ele2]

                removals.forEach((el: Element) => {
    ...
    toSlateRange(editor: VueEditorType, domRange: DOMRange | DOMStaticRange | DOMSelection): Range {
        ...
        const focus = isCollapsed ? anchor : VueEditor.toSlatePoint(editor, [focusNode, focusOffset])
